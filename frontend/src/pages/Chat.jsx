...
// Empty state para el chat
const EmptyState = ({ onCreate }) => (
  <div className="flex flex-col items-center justify-center h-full px-6 py-10 text-center">
    <div className="w-16 h-16 rounded-2xl bg-teal-50 flex items-center justify-center mb-4">
      <MessagesSquare size={28} className="text-teal-500" />
    </div>
    <p className="text-base font-semibold text-gray-700 mb-1">
      Sin conversaciones
    </p>
    <p className="text-sm text-gray-400 mb-6 max-w-xs">
      Crea tu primera consulta y empieza a hablar con el asistente dlAgnose.
    </p>
    <button
      onClick={onCreate}
      className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-teal-600 text-white text-sm
                 font-medium hover:bg-teal-700 active:scale-95 transition-all shadow-sm shadow-teal-200"
    >
      <Plus size={16} />
      Nueva conversación
    </button>
...
const Chat = () => {
  ...
  const handleCreateChat = async (titulo) => {
    try {
      if (!titulo) return;
      ...
      <div className="flex gap-4 h-full overflow-hidden relative">
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto py-2">
            {sessions.length === 0 ? (
              <EmptyState onCreate={() => setShowNewChatModal(true)} />
            ) : (
              sessions.map((s) => (
                <button
                  key={s.id}
